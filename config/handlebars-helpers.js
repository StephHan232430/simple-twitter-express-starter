...
module.exports = {
  ...
  hashtag: function(description) {
    ...
    function hashtagOf(description) {
      var regex = new RegExp(getPattern(), 'ig')
      if (regex.test(description)) {
        return description.match(regex)
      }
    }

    const tags = hashtagOf(description)
    const transformedTags = tags.map(tag => {
      return tag.link('/tweets/{{categoryId}}')
      // href = '/tweets/{{categoryId}}'
    })
    let stringArray = []
    let result = ''

    if (description[0] !== '#') {
      // 開頭string，string先切，再切# ; 拼接時，string先上，再接#
      ...
      // 開頭#，先切tag[i]長度，再切string ; 拼接時，#先上，再接string
      for (let i = 0; i < tags.length; i++) {
        if (description.includes(tags[i])) {
          description = description.substring(0, tags[i].length)
          if (stringArray.substring(0, description.indexOf('#') !== ' ')) {
            stringArray.push(description.substring(0, description.indexOf('#')))
          } else {
            stringArray.push('&nbsp;')
          }
